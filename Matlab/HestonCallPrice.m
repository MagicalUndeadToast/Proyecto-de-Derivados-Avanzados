function [C] = HestonCallPrice(S,K,r,q,T,vt,kap,th,sig,rho,lda)
...
f1 = CF_SVj(log(S),vt,T,r,q,kap*th,0.5,kap+lda-rho*sig,rho,sig,phi);
P1 = 0.5+(1/pi)*sum(real(exp(-1i*phi*log(K)).*f1./(1i*phi))*dphi);
f2 = CF_SVj(log(S),vt,T,r,q,kap*th,-0.5,kap+lda,rho,sig,phi);
